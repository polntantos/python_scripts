...
def annotate_text(title, annotations):
    ...
    for pre_annotation in previous_annotations:
      if(pre_annotation['word'] in target_string  and pre_annotation['word']+"," not in target_string and  not target_string.endswith(pre_annotation['word'])):
        target_string  = target_string.replace(pre_annotation['word'],pre_annotation['word'] + ",")
    # target_string  = target_string.replace('-', ",-,")
    text = st.text_input("Enter the text(split by comma , ):",target_string,key="markup_input")
    ...
    if text:
      ...
      for word in words:
        ...
        previous_annotation = get_previous_annotation(annotation_word, previous_annotations)
...
def main():
    ...
    with open("clusters/prepared-clusters.json") as f:
